...
typedef struct {
    ...
    uint32_t idx;
} secp256k1_silentpayments_tx_output_entry;

enum { SECP256K1_SILENTPAYMENTS_MAX_CACHED_LABELS = 16 };

typedef struct {
    secp256k1_ge spend_ge;
    secp256k1_ge label_ge;
    unsigned char label33[33];
    unsigned char label_tweak[32];
} secp256k1_silentpayments_cached_label;

/* Compare function for lexicographically sorting transaction outputs by their x-only serialization. */
static int secp256k1_silentpayments_tx_output_entry_sort_cmp(const void* e1, const void* e2, void *cmp_data) {
    const secp256k1_silentpayments_tx_output_entry *a = (const secp256k1_silentpayments_tx_output_entry *)e1;
    const secp256k1_silentpayments_tx_output_entry *b = (const secp256k1_silentpayments_tx_output_entry *)e2;
    (void)cmp_data;
    return secp256k1_memcmp_var(a->xonly32, b->xonly32, sizeof(a->xonly32));
}

/* Find a transaction output via binary search in a sorted x-only output list.
 * Returns 1 and sets *found_idx if found, or returns 0 otherwise. */
static int secp256k1_silentpayments_tx_output_find(const secp256k1_silentpayments_tx_output_entry *tx_outputs_sorted, size_t n_tx_outputs, const unsigned char *xonly32, uint32_t *found_idx) {
    size_t low = 0;
    size_t high = n_tx_outputs;

    while (low < high) {
        size_t mid = (low + high) / 2;
        int cmp = secp256k1_memcmp_var(xonly32, tx_outputs_sorted[mid].xonly32, 32);
        if (cmp == 0) {
            *found_idx = tx_outputs_sorted[mid].idx;
            return 1;
        } else if (cmp < 0) {
            high = mid;
        } else {
...
/* Like secp256k1_silentpayments_tx_output_find, but skips outputs that are already used.
 *
 * This also handles duplicate x-only outputs by scanning neighbors for an unused match. */
static int secp256k1_silentpayments_tx_output_find_unused(const secp256k1_silentpayments_tx_output_entry *tx_outputs_sorted, size_t n_tx_outputs, const unsigned char *xonly32, const unsigned char *used, uint32_t *found_idx) {
    size_t low = 0;
    size_t high = n_tx_outputs;
    size_t pos;
...
found:
    if (!used[tx_outputs_sorted[pos].idx]) {
        *found_idx = tx_outputs_sorted[pos].idx;
        return 1;
    }

    /* Duplicate x-only pubkeys are uncommon, but handle them correctly. */
    {
        size_t i = pos;
        while (i > 0) {
            i--;
            if (secp256k1_memcmp_var(xonly32, tx_outputs_sorted[i].xonly32, 32) != 0) break;
            if (!used[tx_outputs_sorted[i].idx]) {
                *found_idx = tx_outputs_sorted[i].idx;
                return 1;
            }
        }
    }
...
int secp256k1_silentpayments_recipient_scan_outputs(
    ...
    /* Precompute and sort transaction outputs by their x-only serialization to find them quickly. */
    tx_outputs_sorted = (secp256k1_silentpayments_tx_output_entry*)checked_malloc(&ctx->error_callback, n_tx_outputs * sizeof(*tx_outputs_sorted));
    if (tx_outputs_sorted == NULL) {
        secp256k1_memclear_explicit(shared_secret, sizeof(shared_secret));
        return 0;
    }
    tx_outputs_used = (unsigned char*)checked_malloc(&ctx->error_callback, n_tx_outputs);
    tx_outputs_unused = (uint32_t*)checked_malloc(&ctx->error_callback, n_tx_outputs * sizeof(*tx_outputs_unused));
    tx_outputs_unused_pos = (uint32_t*)checked_malloc(&ctx->error_callback, n_tx_outputs * sizeof(*tx_outputs_unused_pos));
    if (tx_outputs_used == NULL || tx_outputs_unused == NULL || tx_outputs_unused_pos == NULL) {
    ...
    for (k = 0; k < k_max; k++) {
        ...
        secp256k1_ge label_ge;
        /* Initialize to silence -Wconditional-uninitialized (Clang). */
        uint32_t found_idx = 0;
        size_t variants_len;
        size_t vi;
        secp256k1_gej tweak_gej;
        secp256k1_gej variant_out_gej[1 + SECP256K1_SILENTPAYMENTS_MAX_CACHED_LABELS];
        secp256k1_ge variant_out_ge[1 + SECP256K1_SILENTPAYMENTS_MAX_CACHED_LABELS];
        ...
        found = 0;
        /* Compute tweak*G once, used to test unlabeled spend pubkey and any discovered labeled spend pubkeys. */
        secp256k1_ecmult_gen(&ctx->ecmult_gen_ctx, &tweak_gej, &output_tweak_scalar);
        variants_len = 1 + cached_labels_len;
        /* output for the unlabeled spend pubkey */
        secp256k1_gej_add_ge_var(&variant_out_gej[0], &tweak_gej, &spend_pubkey_ge, NULL);
        /* outputs for cached labeled spend pubkeys */
        ...
        /* This can fail if tweak*G is the negation of spend_pubkey, which happens only with negligible probability
         * for honestly created spend_pubkey. */
        ...
        for (vi = 0; vi < variants_len; vi++) {
            ...
            if (secp256k1_silentpayments_tx_output_find_unused(tx_outputs_sorted, n_tx_outputs, candidate_xonly_ser, tx_outputs_used, &found_idx)) {
                if (vi == 0) {
                    label_tweak = NULL;
                } else {
        ...
        if (!found && label_lookup != NULL) {
            ...
            while (scanned < tx_outputs_unused_len && !found) {
                ...
                secp256k1_ge tx_neg_ge_batch[SECP256K1_SILENTPAYMENTS_LABEL_SCAN_CHUNK];
                secp256k1_gej label_candidates_gej[2 * SECP256K1_SILENTPAYMENTS_LABEL_SCAN_CHUNK];
                secp256k1_ge label_candidates_ge[2 * SECP256K1_SILENTPAYMENTS_LABEL_SCAN_CHUNK];
                size_t ci;
                ...
                while (scanned < tx_outputs_unused_len && chunk_len < SECP256K1_SILENTPAYMENTS_LABEL_SCAN_CHUNK) {
                    ...
                    tx_neg_ge_batch[chunk_len] = tx_ge_batch[chunk_len];
                ...
                for (ci = 0; ci < chunk_len; ci++) {
                     ...
                     */
                    secp256k1_gej_set_ge(&tx_output_gej, &tx_ge_batch[ci]);
                    secp256k1_gej_add_ge_var(&label_candidates_gej[2 * ci], &tx_output_gej, &output_negated_ge, NULL);
                    secp256k1_gej_set_ge(&tx_output_gej, &tx_neg_ge_batch[ci]);
                    secp256k1_gej_add_ge_var(&label_candidates_gej[2 * ci + 1], &tx_output_gej, &output_negated_ge, NULL);
                }
                secp256k1_ge_set_all_gej_var(label_candidates_ge, label_candidates_gej, 2 * chunk_len);

                /* Check if any of the label candidates is in the label cache. */
                for (ci = 0; ci < chunk_len; ci++) {
                    unsigned char label33[33];

                    secp256k1_eckey_pubkey_serialize33(&label_candidates_ge[2 * ci], label33);
                    label_tweak = label_lookup(label33, label_context);
                    if (label_tweak != NULL) {
                        found = 1;
                        found_idx = idxs[ci];
                        label_ge = label_candidates_ge[2 * ci];
                        break;
                    }

                    secp256k1_eckey_pubkey_serialize33(&label_candidates_ge[2 * ci + 1], label33);
                    label_tweak = label_lookup(label33, label_context);
                    if (label_tweak != NULL) {
                        found = 1;
                        found_idx = idxs[ci];
                        label_ge = label_candidates_ge[2 * ci + 1];
                        break;
                    }
                }
            }
            ...
            if (found && label_tweak != NULL) {
                ...
                if (!known) {
                    ...
                    if (cached_labels_len < SECP256K1_SILENTPAYMENTS_MAX_CACHED_LABELS) {
                        ins = cached_labels_len;
                        cached_labels_len++;
                    } else {
                        ins = cached_labels_next;
                        cached_labels_next = (cached_labels_next + 1) % SECP256K1_SILENTPAYMENTS_MAX_CACHED_LABELS;
                    }
        ...
        if (found) {
            ...
            if (tx_outputs_unused_len > 0) {
                const uint32_t found_pos = tx_outputs_unused_pos[found_idx];
                const uint32_t last_idx = tx_outputs_unused[tx_outputs_unused_len - 1];
                tx_outputs_unused[(size_t)found_pos] = last_idx;
                tx_outputs_unused_pos[last_idx] = found_pos;
