...
int secp256k1_silentpayments_recipient_scan_outputs(
    ...
    /* Clear the scan_key_scalar since we no longer need it. */
    secp256k1_scalar_clear(&scan_key_scalar);
    ...
    for (i = 0; i < n_tx_outputs; i++) {
        ...
        if (found) {
            ...
            /* Clear the tweak scalar now. */
            secp256k1_scalar_clear(&output_tweak_scalar);

            if (label_tweak != NULL) {
                found_outputs[n_found]->found_with_label = 1;
                /* If tweak + label_tweak overflows to zero mod n, treat as zero-tweak (still spendable). */
